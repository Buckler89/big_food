...
def manage_barcode(db_class, db_collection, radio_label):
    ...
    elements = models.query_collection(db_class, db_collection, mode='OR',
                                                         **{"name": search_word, "batch_number": search_word})
    ...
    if selection:
        ...
        bar_code = generate_barcode(name_to_batch[selection])
        barcode_image_placeholder.image(bar_code)
        btn = download_button_placeholder.download_button(
                label="Download barcode",
                data=bar_code,
                file_name=f"{selection}.png",
                mime="image/png"
              )
